...
%-------------------------------------------------------------------------%
% % Uncomment this to plot the 'eh' pulse and its dft
...
subplot(2,1,2)
sdft = fft(s_eh);
sdft = sdft(1:(length(s_eh)/2)+1);
ssdx = (1/(2*pi*length(s_eh))) * abs(sdft).^2;
ssdx(2:end-1) = 2*ssdx(2:end-1);
freq = 0:Fs/length(s_eh):Fs/2;
...
%-------------------------------------------------------------------------%
% % Uncomment this to play the 'eh'

eh = audioplayer(s_eh, Fs);
play(eh);
